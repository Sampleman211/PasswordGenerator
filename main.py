...
class App(CTk.CTk):
    def __init__(self):
        ...
        self.logo = CTk.CTkImage(dark_image=Image.open("img.png"), size=(460, 150))  # Создание логотипа
        self.logo_label = CTk.CTkLabel(master=self, text="", image=self.logo)
        self.logo_label.grid(row=0, column=0)

        self.password_frame = CTk.CTkFrame(master=self, fg_color="transparent")   # Создание фрейма для поля пароля
        self.password_frame.grid(row=1, column=0, padx=(20, 20), sticky="nsew")

        self.entry_password = CTk.CTkEntry(master=self.password_frame, width=300)   # Создание поля ввода пароля
        ...
        self.btn_generate = CTk.CTkButton(master=self.password_frame, text="Generate", width=100,
                                          command=self. set_password)   # Создание кнопки "Generate"
        self.btn_generate.grid(row=0,column=1)

        self.settings_frame = CTk.CTkFrame(master=self)   # Создание фрейма для настроек
        ...
        self.password_length_slider.grid(row=1, column=0, columnspan=3, pady=(20, 20), sticky="ew")   # Создание ползунка для выбора длины пароля

        self.password_length_entry = CTk.CTkEntry(master=self.settings_frame, width=50)    # Создание поля ввода для указания длины пароля
        self.password_length_entry.grid(row=1, column=3, pady=(20, 10), sticky="we")

        self.cb_digits_var = tkinter.StringVar()   # Создание флажков для выбора символов пароля
        ...
        self.appearance_mode_option_menu.grid(row=3, column=0, columnspan=4, pady=(10,10))   # Создание выпадающего меню для выбора режима отображения

        self.password_length_slider.set(17)   # Установка начальных значений элементов интерфейса
        self.password_length_entry.insert(0, 17)
